...
#define SERIAL_PARAM0 SERIAL_8N1    // Data/Parity/Stop UART1
#define SERIAL1_TCP_PORT 8881       // Wifi Port UART1

#define bufferSize 1024
#define MAX_NMEA_CLIENTS 4
#define NUM_COM 2
#define DEBUG_COM 1 // debug output to COM1
//////////////////////////////////////////////////////////////////////////
...
uint8_t buf1[NUM_COM][bufferSize]; //useful buffer gotten from wifi
uint16_t i1[NUM_COM] = {0,0};

uint8_t buf2[bufferSize]; //buffer gotten from uart
uint16_t i2[NUM_COM] = {0,0};

uint8_t buf3[NUM_COM][bufferSize]; //full buffer gotten from wifi
  ...
  for (int num = 0; num < NUM_COM ; num++)
  {
    ...
    if (COM[num] != NULL) {
        for (byte cln = 0; cln < MAX_NMEA_CLIENTS; cln++) {        
            if (TCPClient[num][cln]) {
                ...
                if (i3[num] > 0 && buf3[num][0] == '>') {
                    for (i1[num] = 0; i1[num] -1 < i3[num]; i1[num]++) {
                        buf1[num][i1[num]] = buf3[num][i1[num]];
                    }
                    pointer[num] = 1;
                    i3[num] = 0;
                } 
        ...
        if (pointer[num] + 15 < i1[num]) {                  
            ...
            for(byte cln = 0; cln < MAX_NMEA_CLIENTS; cln++) {   
                if(TCPClient[num][cln]) {
                    ...
                    TCPClient[num][cln].println(toWifi + toUART); // and sent info to wifi
                }
            }
            pointer[num] += 16;
            byte tmr = 0;
            ...
            for(byte cln = 0; cln < MAX_NMEA_CLIENTS; cln++) {   
                ...
                else if (TCPClient[num][cln]) TCPClient[num][cln].print("\r\n");
            }  
            if (COM[num]->available()) {
                while (COM[num]->available()) {
                    toWifi = Serial.readString(); // read string from UART(num)
                }
                for(byte cln = 0; cln < MAX_NMEA_CLIENTS; cln++) {
                    if(TCPClient[num][cln]) TCPClient[num][cln].print(toWifi);
                }
            }     
        ...
        if (i3[num] > 2 && i3[num] != 21) { //'\n' ignored       
            ...
            for(byte cln = 0; cln < MAX_NMEA_CLIENTS; cln++) {   
                if(TCPClient[num][cln]) TCPClient[num][cln].println("Ready to go");
            }
        }
        i3[num] = 0;
    }
